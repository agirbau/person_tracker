...
def parse_args():
    ...
    parser.add_argument('--im_names', dest='im_names', nargs='+',
                        help='image_names',
                        default=['2008_000533.jpg'])
    parser.add_argument('--folder_path', dest='im_folder', nargs='+',
                        help='path for the folder containing the images',
                        default=['./data/demo'])
    parser.add_argument('--save_path', dest='save_path', nargs='+',
                        help='path for the folder where to save the object masks',
                        default=['./data/demo'])
...
# MODIFIED FUNCTION TO PROVIDE ONLY PERSONS
def get_vis_dict(result_box, result_mask, img_name, cls_names, vis_thresh=0.5):
    #vis_thresh changes the amount of masks to print
    box_for_img = []
    mask_for_img = []
    cls_for_img = []
    #print cls_names
    cls_ind = cls_names.index('person')
    cls_name = cls_names[cls_ind]
    #print cls_ind
...
# ORIGNIAL FUNCTION RETURNING A SET OF OBJECTS
#def get_vis_dict(result_box, result_mask, img_name, cls_names, vis_thresh=0.5):
#    #vis_thresh changes the amount of masks to print
#    box_for_img = []
#    mask_for_img = []
...
if __name__ == '__main__':
    ...
    #            '2008_001717.jpg', '2008_008093.jpg', '00019.jpg', 'libby_roi.jpg']
    ...
    print im_names[0]
    print save_path[0]
    demo_dir = im_folder[0] 
    save_dir = save_path[0]
    #print demo_dir
...
#	print cls_img.max()
        for i in xrange(img_height):
            for j in xrange(img_width):
                ...
                #cls_out_img[i][j] = color_map[cls_img[i][j]][::1]
                #cls_out_img[i][j] = cls_img[i][j] * 255
        
#	cv2.imwrite(target_cls_file, cls_out_img)
	
	#mask_save = demo_dir + '/' + im_name[:-4] + '_mask.png'
	mask_save = save_dir + '/' + im_name[:-4] + '_mask.png'
	cv2.imwrite(mask_save, cls_out_img)
...
"""
	...
	#mask_save = demo_dir + '/' + im_name[:-4] + '_mask.png'
	#print type(mask)
	#cv2.imwrite(mask_save, mask.convert('RGBA'))
