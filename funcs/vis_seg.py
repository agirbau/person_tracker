...
def _convert_pred_to_image(img_width, img_height, pred_dict):
    ...
    for i in xrange(num_inst):
        ...
        #cls_num = pred_dict['cls_name'][num_inst-i-1] 
	#cls_num = i+1
	#print cls_num
        ...
        # Commented by Andreu, this gives the mask associated to a a certain class (e.g. person is cls_num=15, but it mixes all the persons, and we want everything separated).
        #cls_img[box[1]:box[3]+1, box[0]:box[2]+1] = cls_num * mask.astype(np.float32) + part3 
	cls_img[box[1]:box[3]+1, box[0]:box[2]+1] = cls_num * mask.astype(np.float32) + part3 + part1

        # Plot bounding boxes simultaneously (commented by Andreu because we don't care about bounding boxes, we only want the masks)
